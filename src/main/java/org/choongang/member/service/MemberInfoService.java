...
MemberInfoService implements UserDetailsService {
    ...
    public ListData<Member> getList(MemberSearch search) {
        ...
        int limit = Utils.onlyPositiveNumber(search.getLimit(), 20); // 한 페이지 당 레코드 갯수
        int offset = (page - 1) * limit; // 레코드 시작 위치 번호 20, 40 ...
        ...
        List<Member> items = new JPAQueryFactory(em)
                ...
                .limit(limit)
                .offset(offset)
                .orderBy(new OrderSpecifier(Order.DESC, pathBuilder.get("createdAt"))) // 최근 가입자 순서
                .fetch();
