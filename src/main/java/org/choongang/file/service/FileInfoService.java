...
public class FileInfoService {
    ...
    public FileInfo get(Long seq) {
        FileInfo fileInfo = repository.findById(seq)
                .orElseThrow(FileNotFoundException::new); // 파일 없으면 예외 던지기
    ...
    public List<FileInfo> getList(String gid, String location, String mode) {
        ...
        List<FileInfo> items = (List<FileInfo>) repository
                .findAll(builder, Sort.by(asc("createdAt")));
        items.forEach(this::addFileInfo); // 데이터 넣기
...
/*        String thumbDirCommon = "thumbs/" + dir + "/" + seq;
        ...
        fileInfo.setThumsPath(thumbsPath);
        fileInfo.setThumsUrl(thumbsUrl);
        /* 썸네일 경로, URL E*/
    ...
    public String[] getThumb(long seq, int width, int height) {
        ...
        // thumbUrl
        String thumbUrl = String.format("%s/%d_%d_%s", getThumbUrl(seq), width,height,
                fileName);
